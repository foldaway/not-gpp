...
HRESULT Graphics::loadTexture(const char *filename, COLOR_ARGB transcolor,
	...
	// The struct for reading the file info
	D3DXIMAGE_INFO info;
	result = E_FAIL;
	...
	try {
		...
		result = D3DXCreateTextureFromFileEx(
			...
			D3DPOOL_DEFAULT,	// Mermory class for the texture
...
void Graphics::drawSprite(const SpriteData &spriteData, COLOR_ARGB color) {
	...
	if (spriteData.flipHorizontal) {
		...
		// Flip occurs around left edge, translate right to put flipped image in same location as original
	...
	if (spriteData.flipVertical) {
		...
		// Flip occurs around top edge, translate down to put flipped image in same location as original
