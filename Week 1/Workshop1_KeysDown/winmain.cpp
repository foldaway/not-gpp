...
LRESULT WINAPI WinProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	DWORD chWidth = 20;						// width of character
	DWORD chHeight = 20;					// height of character
	...
	case WM_KEYDOWN:						// key down
		...
		InvalidateRect(hwnd, NULL, TRUE);			// forece WM_PAINT
		return 0;
		break;
	...
	case WM_CHAR:									// a character is entered on keyboard
		switch (wParam)
		{
			case 0x08:		// backspace
			case 0x09:		// tab
	...
	case WM_PAINT:
		...
		// Display the state of vkKeys arrays
		...
		for (int r = 0; r < 16; r++)
		{
			for (int c = 0; c < 16; c++)
			{
				...
				if (vkKeys[r * 16 + c])
				{
					SetBkMode(hdc, OPAQUE);				// OPAQUE text BG
					TextOut(hdc, c*chWidth + chWidth * 2, r*chHeight + chHeight * 2, "T ", 2);
				}
				else
				{
					SetBkMode(hdc, TRANSPARENT);			// Transparent text BG
